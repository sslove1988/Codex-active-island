...
#[cfg(windows)]
const SHORTCUT_MOD_ALT: u32 = MOD_ALT.0;
#[cfg(not(windows))]
const SHORTCUT_MOD_ALT: u32 = 0x0001;
#[cfg(windows)]
const SHORTCUT_MOD_CONTROL: u32 = MOD_CONTROL.0;
#[cfg(not(windows))]
const SHORTCUT_MOD_CONTROL: u32 = 0x0002;
#[cfg(windows)]
const SHORTCUT_MOD_SHIFT: u32 = MOD_SHIFT.0;
#[cfg(not(windows))]
const SHORTCUT_MOD_SHIFT: u32 = 0x0004;
#[cfg(windows)]
const SHORTCUT_MOD_META: u32 = MOD_WIN.0;
#[cfg(not(windows))]
const SHORTCUT_MOD_META: u32 = 0x0008;
...
fn parse_shortcut_binding(shortcut: &str) -> Option<ShortcutBinding> {
    ...
    if has_ctrl {
        ...
        modifiers |= SHORTCUT_MOD_CONTROL;
    }

    if has_alt {
        label_parts.push("Alt".to_string());
        modifiers |= SHORTCUT_MOD_ALT;
    }

    if has_shift {
        label_parts.push("Shift".to_string());
        modifiers |= SHORTCUT_MOD_SHIFT;
    }

    if has_win {
        label_parts.push("Win".to_string());
        modifiers |= SHORTCUT_MOD_META;
    }
